"""
Copied from src/global_state.py, not imported -- GlobalState is the
genuinely stateful core of the whole engine (cash balances, positions, the
historical NAV ledger), used by every src2 backtest, so it gets its own
copy here instead of a src/ import, same reasoning as
iteration/trade_implement.py forking TransactionExecutor. src/global_state.py
is untouched and still backs src/run.py.

The only change from the src/ version: the unused `from src.executor_module
import ExecutedTrade` import is dropped -- it was only ever referenced as a
quoted forward-ref type hint (`trade: "ExecutedTrade"`), never actually
resolved at runtime, so it added a src/ coupling for nothing.
"""
import pandas as pd
import numpy as np
...
class GlobalState:
    ...
    def commit_executed_trade(self, trade) -> None:
        """
        ...
        Duck-typed on `trade` (any object with sleeve/action/ticker/shares/
        gross_value/fee/net_value attributes) -- iteration/trade_implement.py's
        ExecutedTrade satisfies this without needing to import it here.
        """
