...
bootstrap_metrics (from bootstrap_compare.run_one_simulation, one row per
...
src.stat_test's paired t-test / Wilcoxon signed-rank / normality-gated
picker, and adds a win-rate + percentile CI on the per-world differences,
which needs no distributional assumption at all and matches the "ask the
same question across worlds" spirit of the bootstrap.
"""
import numpy as np
import pandas as pd
...
def compare_scorer_vs_benchmark(
    ...
    otherwise -- see src.stat_test.automated_paired_test).
    """
    wide = bootstrap_metrics.pivot(index="sim_id", columns="scorer", values=metric)
    diff = _diff_series(bootstrap_metrics, scorer, bench, metric)

    ci_lo, ci_hi = np.percentile(diff, [2.5, 97.5])
    test_result = stat_test.automated_paired_test(wide[scorer], wide[bench])
...
def compare_scorer_vs_benchmark_one_sided(
    ...
    ci_lo, ci_hi = np.percentile(diff, [2.5, 97.5])
    test_result = stat_test.automated_paired_test_one_sided(wide[scorer], wide[bench], alternative=alternative)
