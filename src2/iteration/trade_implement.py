"""
Trade implementation. This used to be a thin wrapper around
src.executor_module.TransactionExecutor; the actual executor logic is now
inlined here instead, since the "wrapper" added an import with no real
abstraction on top of it (see the src/executor_module.py history for why
it grew an AD_HOC_BUY branch that only this copy needs to stay in sync
with). src/executor_module.py is untouched and still backs src/run.py --
this is an intentional fork, not a shared dependency, so src2 no longer
needs src.executor_module / src.logging / src.signal_schema at all.

ExecutedTrade + TransactionExecutor: evaluates whatever
    ...
    def execute_trades(self, state, today: pd.Timestamp, current_prices: pd.Series):
        ...
        if is_buy_adhoc:
            ...
            for signal in buy_adhoc:
                ...
                # (allocation_tactical.py's sizing_fn), e.g. 0.10 for "10% of tactical NAV"
...
def implement_trades(executor: TransactionExecutor, g_state, today: pd.Timestamp, market_prices: pd.Series):
